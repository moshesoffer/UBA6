...
//#define ETX_OTA_DATA_MAX_SIZE ( 32 * 1024 )  //Maximum data Size
#define ETX_OTA_DATA_MAX_SIZE ( 8 * 0x1000 )  //Maximum data Size
...
typedef enum
{
  ...
  ETX_OTA_PACKET_TYPE_PRINT     = 4,    // Print
...
typedef struct
{
	...
	uint16_t    data_len;
	uint8_t     data [256];
}/*__attribute__((packed))*/ ETX_OTA_PRINT_;
#pragma pack(pop)

ETX_OTA_EX_ etx_ota_download_and_flash( void );
void etx_ota_send_print( const char *format, ...);
