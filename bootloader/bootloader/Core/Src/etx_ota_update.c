/*
 * etx_ota_update.c
 *
 *  Created on: 26-Jul-2021
 *      Author: EmbeTronicX
 */

#include <stdio.h>
#include "main.h"
#include <string.h>
#include <stdbool.h>
#include <stdarg.h>

#include "stm32g4xx_hal_flash.h"

#include "etx_ota_update.h"

#define RCV_MIN_DELAY      0x400
#define RCV_MAX_DELAY      0x4000
#define SND_MAX_DELAY      0x4000

#define FLASH_ROW_SIZE          256U      // bytes
#define FLASH_DOUBLEWORD_SIZE   8U
#define FLASH_ROW_DWORDS        32U        // 256 / 8

/* Buffer to hold the received data */
static uint8_t Rx_Buffer[ ETX_OTA_PACKET_MAX_SIZE ];
static uint8_t Tx_Buffer[ 512 ];

/* OTA State */
static ETX_OTA_STATE_ ota_state = ETX_OTA_STATE_IDLE;

/* Firmware Total Size that we are going to receive */
static uint32_t ota_fw_total_size;
/* Firmware image's CRC32 */
static uint32_t ota_fw_crc;
/* Firmware Size that we have received */
static uint32_t ota_fw_received_size;

static uint16_t etx_receive_chunk( uint8_t *buf, uint16_t max_len );
static ETX_OTA_EX_ etx_process_data( uint8_t *buf, uint16_t len );
static void etx_ota_send_resp( uint8_t type );
static HAL_StatusTypeDef write_data_to_flash_app( uint8_t *data,
                                        uint16_t data_len, bool is_full_image );
/*static*/ void etx_ota_send_print( const char *format, ...);

//additional declarations
static uint64_t PackDoubleWord(const uint8_t *p);
HAL_StatusTypeDef Flash_WriteBuffer_DoubleWord(
    uint32_t flashAddr,
    const uint8_t *data,
    uint32_t length);
HAL_StatusTypeDef Flash_ProgramFastRow(
    uint32_t flashAddress,
    const uint8_t *data256);

                                        
/**
  * @brief Download the application from UART and flash it.
  * @param None
  * @retval ETX_OTA_EX_
  */
ETX_OTA_EX_ etx_ota_download_and_flash( void )
{
  ETX_OTA_EX_ ret  = ETX_OTA_EX_OK;
  uint16_t    len;
  int retries = 4;

  uart_printf("Waiting for the OTA data...\r\n");
  //etx_ota_send_print("Waiting for the OTA data...\r\n");

  /* Reset the variables */
  ota_fw_total_size    = 0u;
  ota_fw_received_size = 0u;
  ota_fw_crc           = 0u;
  ota_state            = ETX_OTA_STATE_START;

  do
  {
    //clear the buffer
    memset( Rx_Buffer, 0, ETX_OTA_PACKET_MAX_SIZE );

#if 0
    len = etx_receive_chunk( Rx_Buffer, ETX_OTA_PACKET_MAX_SIZE );
    //uart_printf("etx_receive_chunk, len %d\r\n", len);
#else
    while (retries)
    {
      len = etx_receive_chunk( Rx_Buffer, ETX_OTA_PACKET_MAX_SIZE );
      //uart_printf("etx_receive_chunk, len %d, retries %d\r\n", (int16_t)len, retries);
      if ( ( (int16_t)len < 0 ) && 
           ( retries > 0) ) // timeout, retry receive chunck
      {
        uart_printf("-");
        etx_ota_send_print("Start Programming\r\n");
        retries--;
        continue;
      }

      retries = 1;
      break;
    }
#endif
    if (retries == 0)
    {
      uart_printf("jump to applic\r\n");
      return ETX_OTA_EX_OK;//let jump to application
    }

    if( len != 0u )
    {
      ret = etx_process_data( Rx_Buffer, len );
    }
    else
    {
      //didn't received data. break.
      ret = ETX_OTA_EX_ERR;
    }

    //Send ACK or NACK
    if( ret != ETX_OTA_EX_OK )
    {
      uart_printf("Sending NACK\r\n");
      etx_ota_send_resp( ETX_OTA_NACK );
      break;
    }
    else
    {
      //uart_printf("Sending ACK\r\n");
      etx_ota_send_resp( ETX_OTA_ACK );
    }

  }while( ota_state != ETX_OTA_STATE_IDLE );

  return ret;
}

/**
  * @brief Process the received data from UART4.
  * @param buf buffer to store the received data
  * @param max_len maximum length to receive
  * @retval ETX_OTA_EX_
  */
static ETX_OTA_EX_ etx_process_data( uint8_t *buf, uint16_t len )
{
  ETX_OTA_EX_ ret = ETX_OTA_EX_ERR;

  do
  {
    if( ( buf == NULL ) || ( len == 0u) )
    {
      break;
    }

    //Check we received OTA Abort command
    ETX_OTA_COMMAND_ *cmd = (ETX_OTA_COMMAND_*)buf;
    if( cmd->packet_type == ETX_OTA_PACKET_TYPE_CMD )
    {
      if( cmd->cmd == ETX_OTA_CMD_ABORT )
      {
        //received OTA Abort command. Stop the process
        break;
      }
    }

    switch( ota_state )
    {
      case ETX_OTA_STATE_IDLE:
      {
        //uart_printf("ETX_OTA_STATE_IDLE...\r\n");
        ret = ETX_OTA_EX_OK;
      }
      break;

      case ETX_OTA_STATE_START:
      {
        ETX_OTA_COMMAND_ *cmd = (ETX_OTA_COMMAND_*)buf;

        if( cmd->packet_type == ETX_OTA_PACKET_TYPE_CMD )
        {
          if( cmd->cmd == ETX_OTA_CMD_START )
          {
            //uart_printf("Received OTA START Command\r\n");
            ota_state = ETX_OTA_STATE_HEADER;
            ret = ETX_OTA_EX_OK;
          }
        }
      }
      break;

      case ETX_OTA_STATE_HEADER:
      {
        ETX_OTA_HEADER_ *header = (ETX_OTA_HEADER_*)buf;
        if( header->packet_type == ETX_OTA_PACKET_TYPE_HEADER )
        {
          ota_fw_total_size = header->meta_data.package_size;
          ota_fw_crc        = header->meta_data.package_crc;
          //uart_printf("Received OTA Header. FW Size = %ld\r\n", ota_fw_total_size);
          ota_state = ETX_OTA_STATE_DATA;
          ret = ETX_OTA_EX_OK;
        }
      }
      break;

      case ETX_OTA_STATE_DATA:
      {
        ETX_OTA_DATA_     *data     = (ETX_OTA_DATA_*)buf;
        uint16_t          data_len = data->data_len;
        HAL_StatusTypeDef ex;

        if( data->packet_type == ETX_OTA_PACKET_TYPE_DATA )
        {
          /* write the chunk to the Flash (App location) */
          ex = write_data_to_flash_app( buf+4, data_len, ( ota_fw_received_size == 0) );

          if( ex == HAL_OK )
          {
            uart_printf(".");
            etx_ota_send_print(".");

            //uart_printf("[%ld/%ld]\r\n", ota_fw_received_size/ETX_OTA_DATA_MAX_SIZE, ota_fw_total_size/ETX_OTA_DATA_MAX_SIZE);
            if( ota_fw_received_size >= ota_fw_total_size )
            {
              //received the full data. So, move to end
              ota_state = ETX_OTA_STATE_END;
            }
            ret = ETX_OTA_EX_OK;
          }
        }
      }
      break;

      case ETX_OTA_STATE_END:
      {

        ETX_OTA_COMMAND_ *cmd = (ETX_OTA_COMMAND_*)buf;

        if( cmd->packet_type == ETX_OTA_PACKET_TYPE_CMD )
        {
          if( cmd->cmd == ETX_OTA_CMD_END )
          {
            //uart_printf("Received OTA END Command\r\n");

            //TODO: Very full package CRC

            ota_state = ETX_OTA_STATE_IDLE;
            ret = ETX_OTA_EX_OK;
          }
        }
      }
      break;

      default:
      {
        /* Should not come here */
        ret = ETX_OTA_EX_ERR;
      }
      break;
    };
  }while( false );

  return ret;
}

/**
  * @brief Receive a one chunk of data.
  * @param buf buffer to store the received data
  * @param max_len maximum length to receive
  * @retval ETX_OTA_EX_
  */
static uint16_t etx_receive_chunk( uint8_t *buf, uint16_t max_len )
{
  int16_t  ret;
  uint16_t index     = 0u;
  uint16_t data_len;

  do
  {
    //receive SOF byte (1byte)
    ret = HAL_UART_Receive( &huart1, &buf[index], 1, RCV_MIN_DELAY );
    if( ret != HAL_OK )
    {
      //Timeout
      return -1;
    }

    if( buf[index++] != ETX_OTA_SOF )
    {
      //Not received start of frame
      return 0; 
    }

    //Receive the packet type (1byte).
    ret = HAL_UART_Receive( &huart1, &buf[index++], 1, RCV_MAX_DELAY );
    if( ret != HAL_OK )
    {
      break;
    }

    //Get the data length (2bytes).
    ret = HAL_UART_Receive( &huart1, &buf[index], 2, RCV_MAX_DELAY );
    if( ret != HAL_OK )
    {
      break;
    }
    data_len = *(uint16_t *)&buf[index];
    index += 2u;

    for( uint16_t i = 0u; i < data_len; i++ )
    {
      ret = HAL_UART_Receive( &huart1, &buf[index++], 1, RCV_MAX_DELAY );
      if( ret != HAL_OK )
      {
        break;
      }
    }

    //Get the CRC.
    ret = HAL_UART_Receive( &huart1, &buf[index], 4, RCV_MAX_DELAY );
    if( ret != HAL_OK )
    {
      break;
    }
    index += 4u;

    //TODO: Add CRC verification

    //receive EOF byte (1byte)
    ret = HAL_UART_Receive( &huart1, &buf[index], 1, RCV_MAX_DELAY );
    if( ret != HAL_OK )
    {
      break;
    }

    if( buf[index++] != ETX_OTA_EOF )
    {
      //Not received end of frame
      ret = ETX_OTA_EX_ERR;
      break;
    }

  }while( false );

  if( ret != HAL_OK )
  {
    //clear the index if error
    index = 0u;
  }

  if( max_len < index )
  {
    uart_printf("Received more data than expected. Expected = %d, Received = %d\r\n",
                                                              max_len, index );
    index = 0u;
  }

  return index;
}

/**
  * @brief Send the response.
  * @param type ACK or NACK
  * @retval none
  */
static void etx_ota_send_resp( uint8_t type )
{
  ETX_OTA_RESP_ rsp =
  {
    .sof         = ETX_OTA_SOF,
    .packet_type = ETX_OTA_PACKET_TYPE_RESPONSE,
    .data_len    = 1u,
    .status      = type,
    .crc         = 0u,                //TODO: Add CRC
    .eof         = ETX_OTA_EOF
  };

  //send response
  HAL_UART_Transmit(&huart1, (uint8_t *)&rsp, sizeof(ETX_OTA_RESP_), HAL_MAX_DELAY);
}

  /**
  * @brief Send buffer to be print on screen.
  * @param type ACK or NACK
  * @retval none
  */
/*static*/ void etx_ota_send_print( const char *format, ...)
{
  ETX_OTA_PRINT_ print =
  {
    .sof         = ETX_OTA_SOF,
    .packet_type = ETX_OTA_PACKET_TYPE_PRINT,
    .data_len    = 0,
//  .data        = Tx_Buffer,
  };
  
	va_list args;

	memset(print.data, 0, sizeof((char*)print.data));
	va_start(args, format);
	vsprintf(print.data, format, args);
	perror(print.data);
	va_end(args);

	print.data_len = strlen ((char*) print.data),

  //send response
  HAL_UART_Transmit(&huart1, (uint8_t *)&print, sizeof(ETX_OTA_PRINT_), SND_MAX_DELAY);
	
  return (uint8_t) strlen(print.data);
}

/**
  * @brief Write data to the Application's actual flash location.
  * @param data data to be written
  * @param data_len data length
  * @is_first_block true - if this is first block, false - not first block
  * @retval HAL_StatusTypeDef
  */
static HAL_StatusTypeDef write_data_to_flash_app( uint8_t *data,
                                        uint16_t data_len, bool is_first_block )
{
  HAL_StatusTypeDef ret;

  do
  {
    ret = HAL_FLASH_Unlock();
    if( ret != HAL_OK )
    {
      break;
    }

    //No need to erase every time. Erase only the first time.
    if( is_first_block )
    {

      //uart_printf("Erasing the Flash memory...\r\n");
      //Erase the Flash
      FLASH_EraseInitTypeDef EraseInitStruct;
      uint32_t pageError;

      EraseInitStruct.TypeErase = FLASH_TYPEERASE_PAGES;
      EraseInitStruct.Banks     = FLASH_BANK_2;
      EraseInitStruct.Page     	= 1;                    //erase 2 sectors(5,6)
      EraseInitStruct.NbPages  	= FLASH_PAGE_NB;

      ret = HAL_FLASHEx_Erase( &EraseInitStruct, &pageError );
      if( ret != HAL_OK )
      {
        uart_printf("Flash erase Error %d\r\n", pageError);
        break;
      }
    }

    for(int i = 0; i < data_len; i += data_len )
    {
      //uart_printf("PROGRAMMING the Flash memory... %d, len %d\r\n", i, data_len);
#if 0
      ret = HAL_FLASH_Program( FLASH_TYPEPROGRAM_DOUBLEWORD,
                               (ETX_APP_FLASH_ADDR + ota_fw_received_size),
                               data[i]
                             );
#else
      ret = Flash_WriteBuffer_DoubleWord(
              ETX_APP_FLASH_ADDR + ota_fw_received_size + i,
              data,
              data_len);

      //for (int i=0; i < data_len/FLASH_ROW_SIZE; i++)
      //  ret = Flash_ProgramFastRow(ETX_APP_FLASH_ADDR + ota_fw_received_size + i*FLASH_ROW_SIZE, 
      //        &data[i*FLASH_ROW_SIZE]);
#endif

      if( ret == HAL_OK )
      {
        //update the data count
        ota_fw_received_size += data_len;
      }
      else
      {
        uart_printf("Flash Write Error\r\n");
        break;
      }
    }

    if( ret != HAL_OK )
    {
      break;
    }

    ret = HAL_FLASH_Lock();
    if( ret != HAL_OK )
    {
      break;
    }
  }while( false );

  return ret;
}

//aaditional functions
static uint64_t PackDoubleWord(const uint8_t *p)
{
    return  ((uint64_t)p[0])       |
           (((uint64_t)p[1]) << 8) |
           (((uint64_t)p[2]) << 16)|
           (((uint64_t)p[3]) << 24)|
           (((uint64_t)p[4]) << 32)|
           (((uint64_t)p[5]) << 40)|
           (((uint64_t)p[6]) << 48)|
           (((uint64_t)p[7]) << 56);
}

HAL_StatusTypeDef Flash_WriteBuffer_DoubleWord(
    uint32_t flashAddr,
    const uint8_t *data,
    uint32_t length)
{
    HAL_StatusTypeDef status = HAL_OK;

    // Address must be 8-byte aligned
    if (flashAddr % 8 != 0)
        return HAL_ERROR;

    HAL_FLASH_Unlock();

    for (uint32_t i = 0; i < length; i += 8)
    {
        uint64_t dword = 0xFFFFFFFFFFFFFFFFULL;

        // Handle last partial double-word safely
        for (uint32_t b = 0; b < 8 && (i + b) < length; b++)
        {
            dword &= ~((uint64_t)0xFF << (b * 8));
            dword |= ((uint64_t)data[i + b] << (b * 8));
        }

        status = HAL_FLASH_Program(
                    FLASH_TYPEPROGRAM_DOUBLEWORD,
                    flashAddr,
                    dword);

        if (status != HAL_OK)
            break;

        flashAddr += 8;
    }

    HAL_FLASH_Lock();
    return status;
}

HAL_StatusTypeDef Flash_ProgramFastRow(
    uint32_t flashAddress,
    const uint8_t *data256)
{
    HAL_StatusTypeDef status;
    uint64_t rowData[FLASH_ROW_DWORDS];

    // Address must be 256-byte aligned
    if (flashAddress % FLASH_ROW_SIZE != 0)
        return HAL_ERROR;

    // Pack uint8_t → uint64_t (little-endian)
    for (uint32_t i = 0; i < FLASH_ROW_DWORDS; i++)
    {
        rowData[i] =
            ((uint64_t)data256[i*8 + 0])       |
            ((uint64_t)data256[i*8 + 1] << 8)  |
            ((uint64_t)data256[i*8 + 2] << 16) |
            ((uint64_t)data256[i*8 + 3] << 24) |
            ((uint64_t)data256[i*8 + 4] << 32) |
            ((uint64_t)data256[i*8 + 5] << 40) |
            ((uint64_t)data256[i*8 + 6] << 48) |
            ((uint64_t)data256[i*8 + 7] << 56);
    }

    HAL_FLASH_Unlock();

    __disable_irq();  // strongly recommended

    status = HAL_FLASH_Program(
                FLASH_TYPEPROGRAM_FAST,
                flashAddress,
                (uint64_t)rowData);

    __enable_irq();

    HAL_FLASH_Lock();

    return status;
}
