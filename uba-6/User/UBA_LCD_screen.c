...
void UBA_LCD_screen_load_channel(UBA_LCD_channel *lcd_ch, UBA_channel *ch) {
	...
	sprintf(buffer, "%3ld\xf8 %3s", UBA_channel_get_temperature(ch), "C");
	...
	if (((screen->bpt)) != NULL) {
		...
		if (strlen(channle_test->name)) {
			sprintf(lcd_bpt->test_name.elemnt.text.text, "%.*s",
					UBA_LCD_MIN(strlen(channle_test->name),
...
// EWI line
	...
	if ((screen->bpt->ewi & UBA_BPT_EWI_LEVEL_CRITICAL) == UBA_BPT_EWI_LEVEL_CRITICAL) {
	...
	} else if ((screen->bpt->ewi & UBA_BPT_EWI_LEVEL_ERROR) == UBA_BPT_EWI_LEVEL_ERROR) {
		lcd_bpt->EWI_msg.elemnt.text.color_bg = UBA_GFX_COLOR_WHITE;
		lcd_bpt->EWI_msg.elemnt.text.color_text = UBA_GFX_COLOR_RED;
		lcd_bpt->EWI_msg.effect = UBA_GFX_EFFECT_SOLID;
		snprintf(lcd_bpt->EWI_msg.elemnt.text.text, 10, "ERR:%04u", screen->bpt->ewi);
	} else if ((screen->bpt->ewi & UBA_BPT_EWI_LEVEL_WARNING) == UBA_BPT_EWI_LEVEL_WARNING) {
		lcd_bpt->EWI_msg.elemnt.text.color_bg = UBA_GFX_COLOR_YELLOW;
		lcd_bpt->EWI_msg.elemnt.text.color_text = UBA_GFX_COLOR_BLACK;
		lcd_bpt->EWI_msg.effect = UBA_GFX_EFFECT_SOLID;
		snprintf(lcd_bpt->EWI_msg.elemnt.text.text, UBA_EWI_MAX_LINE_CHAR_SIZE, "WARN:%04u", screen->bpt->ewi);
	} else if ((screen->bpt->ewi & UBA_BPT_EWI_LEVEL_INFO) == UBA_BPT_EWI_LEVEL_INFO) {
		lcd_bpt->EWI_msg.elemnt.text.color_bg = UBA_GFX_COLOR_BLACK;
		lcd_bpt->EWI_msg.elemnt.text.color_text = UBA_GFX_COLOR_WHITE;
		lcd_bpt->EWI_msg.effect = UBA_GFX_EFFECT_SOLID;
		snprintf(lcd_bpt->EWI_msg.elemnt.text.text, UBA_EWI_MAX_LINE_CHAR_SIZE, "INFO:%04u", screen->bpt->ewi);
	} else {
		lcd_bpt->EWI_msg.elemnt.text.color_bg = UBA_GFX_COLOR_WHITE;
		lcd_bpt->EWI_msg.elemnt.text.color_text = UBA_GFX_COLOR_WHITE;
		lcd_bpt->EWI_msg.effect = UBA_GFX_EFFECT_SOLID;
	...
	if ((screen->bpt->ewi & UBA_BPT_EWI_TEST_FAIL) == UBA_BPT_EWI_TEST_FAIL) {
		snprintf(lcd_bpt->EWI_msg.elemnt.text.text, UBA_EWI_MAX_LINE_CHAR_SIZE, " FAILED ");
	}
	if ((screen->bpt->ewi & UBA_BPT_EWI_USER_ABORT) == UBA_BPT_EWI_USER_ABORT) {
		snprintf(lcd_bpt->EWI_msg.elemnt.text.text, UBA_EWI_MAX_LINE_CHAR_SIZE, " ABORT ");
	}
...
void UBA_LCD_screen_display_test_select_enter(UBA_LCD_screen *screen) {
	...
	for (i = 0; i < UBA_LCD_MAX_DISPLAY_TEST_SELECT; i++) {
		...
		sprintf(screen->pages.test_list.test_name_list[i].elemnt.text.text, "%c-%.*s", TR_list[i].type == UBA_BPT_TYPE_SINGLE_CHANNEL ? 'S' : 'D',
				UBA_LCD_screen_line_max_str_length(screen, screen->pages.test_list.test_name_list[i].elemnt.text.size) - 2, TR_list[i].name);
	}
...
void UBA_LCD_screen_display_test_select(UBA_LCD_screen *screen) {
	...
	if (UBA_button_is_pending(screen->main_buttons.bnt_up_p) || UBA_button_is_pending(screen->secondery_buttons.bnt_up_p)) {
		...
		if (screen->pages.test_list.list_select_index < UBA_TR_LIST_SIZE) {
			/*if (TR_list[screen->pages.test_list.list_select_index].type == UBA_BPT_TYPE_DUAL_CHANNEL) {

			 }*/
			screen->tr = &TR_list[screen->pages.test_list.list_select_index];
	...
	if ((screen->pages.test_list.select_index >= UBA_LCD_MAX_DISPLAY_TEST_SELECT - 1)
		...
		for (i = 0; i < UBA_LCD_MAX_DISPLAY_TEST_SELECT; i++) {
			...
			sprintf(screen->pages.test_list.test_name_list[i].elemnt.text.text, "%c-%.*s",
					TR_list[j].type == UBA_BPT_TYPE_SINGLE_CHANNEL ? 'S' : 'D',
					UBA_LCD_screen_line_max_str_length(screen, screen->pages.test_list.test_name_list[i].elemnt.text.size) - 2, TR_list[j].name);
		}
	}
	if (need_to_refresh) {
...
void UBA_LCD_screen_display_test_info(UBA_LCD_screen *screen) {
	...
	if (UBA_button_is_pending(screen->main_buttons.bnt_select_p) || UBA_button_is_pending(screen->secondery_buttons.bnt_select_p)) {
		if (screen->pages.test_info.bnt_back.effect == UBA_GFX_EFFECT_SELECTED) {
			...
			if (TR_list[screen->tr_list_select_index].type == UBA_BPT_TYPE_DUAL_CHANNEL) {
				if (UBA_TR_unpack(&TR_list[screen->tr_list_select_index], &(UBA_6_device_g.BPT_AB)) != 0) {
				...
				if (screen->ch_control == UBA_CHANNLE_ID_AB) {
					if (UBA_button_is_pending(screen->main_buttons.bnt_select_p)) {
						if (UBA_TR_unpack(&TR_list[screen->tr_list_select_index], &(UBA_6_device_g.BPT_A)) != 0) {
							UART_LOG_CRITICAL(UBA_COMP, "TR unpack Failed");
						}
					} else if (UBA_button_is_pending(screen->secondery_buttons.bnt_select_p)) {
						if (UBA_TR_unpack(&TR_list[screen->tr_list_select_index], &(UBA_6_device_g.BPT_B)) != 0) {
							UART_LOG_CRITICAL(UBA_COMP, "TR unpack Failed");
						}
					}
