...
{
	BYTE rxDat;
    if (HAL_SPI_TransmitReceive(&SD_SPI_HANDLE, &dat, &rxDat, 1, 50) != HAL_OK){
    	UBA_6_log("HAL_SPI_TransmitReceive with Error : %lx", HAL_SPI_TransmitReceive(&SD_SPI_HANDLE, &dat, &rxDat, 1, 50));
    }
    return rxDat;
}
