...
#define MCP47X6_MAX_VALUE 		(0xfff) /*the max value that the mcp can be at */
#define MCP47X6_MIN_VALUE 		(0x32) /*the max value that the mcp can be at */

// @formatter:on

typedef enum MCP47X6_StatusTypeDef {
	MCP47X6_STATUS_OK = 0x00,
	MCP47X6_STATUS_INTERFACE_ERROR = 0x01,
	MCP47X6_STATUS_VALUE_ERROR = 2,
	MCP47X6_STATUS_NULL_ERROR = 3,
	MCP47X6_STATUS_BUSY = 4,

} MCP47X6_StatusTypeDef;
...
{									\
	NULL,							\
	MCP47X6_ADDRESS_BASE|MCP47X6_ADDRESS_A1,			\
	MCP47X6_POWER_DOWN_OP_NORMAL,	\
	0,								\
	0,								\
	MCP47X6_RESOLUTION_12BIT,		\
}
